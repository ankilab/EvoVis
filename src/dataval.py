...
def validate_search_space(run):
    ...
    if "rule_set" not in data:
        return (
            "Error search_space.json file: Missing 'rule_set' key in search_space.json."
        )
    ...
    for group, genes in gene_pool.items():
        if not isinstance(genes, list):
            message += (
                f"Error search_space.json file: Gene pool '{group}' must be a list.\n"
            )
        else:
            for gene in genes:
    ...
    if not start_found:
        message += (
            "Error search_space.json file: Rule entry with layer 'Start' is missing.\n"
        )
...
def validate_generations_of_individuals(run):
    ...
    generation_directories = [
        d for d in os.listdir(run) if re.match(r"^Generation_\d+$", d)
    ]

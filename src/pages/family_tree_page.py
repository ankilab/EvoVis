...
def generation_slider():
    ...
    random_generation, _ = get_random_individual(
...
def individual_select():
    ...
    return dmc.Select(
        ...
        icon=DashIconify(
            icon="material-symbols-light:circle", height=10, width=10, color="#6173E9"
        ),
        id="ind-select",
...
def set_individuals_select(gen_range):
    ...
    data = [
        ...
        for ind in get_individuals(
            run,
            generation_range=range(gen, gen + 1),
            value="names",
...
def set_cytoscape(gen_range, ind, ind_clicked, edge_clicked):
    ...
    cytoscape_layout = {"name": "breadthfirst", "roots": roots}
...
@callback(
    Output("individual-heading", "children"),
    Output("individual-exceptions", "children"),
    Output("individual-genes", "children"),
    Output("individual-results", "children"),
    Input("cytoscape-family-tree", "tapNodeData"),
    Input("ind-select", "value"),
    Input("gen-range-slider", "value"),
)
def set_values(ind_clicked, ind_select, gen_range):
    ...
    if "error" in ind_meas and (
        ind_meas["error"] == "True" or ind_meas["error"] == True
    ):
        ind_exceptions.append(warning("Individual errored."))

    val_msg = validate_individual_result(
        run, gen, ind
    ) + validate_individual_chromosome(run, gen, ind)
    ...
    ### 3 CHROMOSOME ###
    ind_genes = [
        dot_heading("Genes", style={"margin": "10px", "flex": "100%"}),
        chromosome_sequence(chromosome=ind_genome),
    ]
    ...
    if "fitness" in ind_meas:
        ind_fitness += [
            bullet_chart_card_basic(
                ind_meas["fitness"], 0, 1, metric_card_id="bullet-chart-basic"
            )
        ]
...
def family_tree_layout():
    ...
    """
    validation_result = validate_generations_of_individuals(
        run
    ) + validate_crossover_parents(run)
    validation_result_ind_inf = validate_meas_info(run)
    layout = None

    if validation_result:
        layout = html.Div(children=[family_tree_header(), warning(validation_result)])
        ...
        if validation_result_ind_inf:
            layout = dmc.Grid(
                children=[
                    family_tree(),
                    dmc.Col(
                        html.Div([warning(validation_result_ind_inf)]),
                        span="auto",
                        style={"max-width": "100%"},
                    ),
                ],
            ...
            layout = dmc.Grid(
                children=[family_tree(), individual_information()],
                gutter="s",
                grow=True,
            )

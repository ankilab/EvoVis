...
def add_meas_trace(
    ...
    gen_results, _ = get_healthy_individuals_results(
        run, generation_range, as_generation_dict=True
    )
...
def figure_meas_over_gen(
    ...
    if len(measures) == 0:
        ...
        fig = go.Figure(
            go.Scatter(x=gens, mode="markers", marker=dict(color="rgba(0,0,0,0)"))
        )
    ...
    # Measure traces
        ...
        for idx, meas in enumerate(measures):
            ...
            add_meas_trace(
                fig, run, meas, generation_range, min, max, show_std, linecolor
            )
...
def get_pareto_optimality_fig(run, generation_range=None, max_width=600, height=200):
    ...
    fig.add_trace(go.Scatter(x=obj1, y=obj2, mode="markers", marker=marker))
    ...
    fig.update_layout(
        ...
        title_font=dict(family="sans-serif"),
        xaxis={
            "title": f"{measurements[fitness_objectives[0]]['displayname']} [{measurements[fitness_objectives[0]]['unit']}]",
            "showline": True,
            "tickfont": tickfont,
            "title_font": titlefont,
        },
        yaxis={
            "title": f"{measurements[fitness_objectives[1]]['displayname']} [{measurements[fitness_objectives[1]]['unit']}]",
            "showgrid": True,
            "gridcolor": "#D0D0D0",
            "tickfont": tickfont,
            "title_font": titlefont,
...
def general_overview():
    ...
    gen_processed = bullet_chart_card_basic(
        processed_gen,
        1,
        tot_gen,
        unit="Generations processed",
        info="Generations",
        back_color="#6173E9",
        bar_color="#A4B0FE",
        load_color="#FFFFFF",
    ...
    ind_healthy = parameter_card(
        "Healthy Individuals",
        len(healthy),
        icon="icon-park-outline:health",
        margin="0px",
        width="100%",
    )
    ind_unhealthy = parameter_card(
    ...
    fitness_plot = graph_meas_over_gen(
        run,
        "fitness",
        generation_range=None,
        min=0,
    ...
    general_overview = dmc.Grid(
        [
            dmc.Col(
                dmc.Stack([gen_processed, ind_healthy, ind_unhealthy]), span="auto"
            ),
...
def objectives_overview():
                ...
                dmc.Col(
                    [
                        dot_heading(
                            ...
                            className="dot-heading-results-page",
                        ),
                        graph_meas_over_gen(
                            run,
                            measurement,
                            generation_range=None,
                            min=meas_info.get("min-boundary", None),
                            max=meas_info.get("max-boundary", None),
                            height=fitn_obj_height,
                            width=250,
                        ),
                    ],
    ...
    objectives_overview = dmc.Grid(
        objective_trends, gutter=grid_gutter, grow=True, justify="flex-start"
    )
...
def best_individuals_overview():
    ...
    for gen, ind in best_individuals.items():
        ...
        ind_overview = html.Div(
            [
                ...
                html.P(
                    f"GEN {gen}",
                    style={"margin": "5px", "font-weight": "bold", "font-size": "11px"},
                ),
                chromosome_sequence(
                    ind["chromosome"],
                    justify="flex-start",
    ...
    genomes_div = dmc.Group(
        genomes, position="left", align="start", style={"gap": "5px"}
    )
...
def run_results_layout():
    ...
    """
    validation_result = validate_generations_of_individuals(run) + validate_meas_info(
        run
    )
    layout = None
    ...
    if validation_result:
        layout = html.Div(
            children=[
                html.H1(
                    "Run Results", style={"margin-bottom": "25px", "margin-top": "25px"}
                ),
